...
export function createSocketServer(httpServer: HttpServer): SocketIOServer {
  const io = new SocketIOServer(httpServer, {
    cors: {
      // Accept any origin — auth is enforced by the io.use() middleware,
      // not by CORS. This matches Obliance's pattern and avoids issues
      // behind reverse proxies where CLIENT_ORIGIN may not match the
      // actual browser origin (http vs https, port mismatch, etc.).
      origin: true,
      credentials: true,
    },
