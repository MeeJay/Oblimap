...
router.get('/', requireAuth, requireRole('admin'), async (_req: Request, res: Response) => {
  ...
  // Probe version from VERSION file (injected into Docker image at build time)
  let probeVersion = 'unknown';
  const probeVersionFile = join(process.cwd(), '..', 'probe', 'VERSION');
  try {
    if (existsSync(probeVersionFile)) {
      probeVersion = readFileSync(probeVersionFile, 'utf-8').trim();
    }
  } catch { /* ignore */ }
