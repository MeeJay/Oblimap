/**
 * SSO routes — cross-app "foreign user" authentication between Oblimap and Obliguard.
 *
 * Endpoints exposed FOR the other app (Bearer auth = shared Obliguard secret):
 *   GET  /api/sso/validate-token?token=xxx   — validate a switch token generated by Oblimap
 *   GET  /api/sso/users                      — list Oblimap users (for admin pre-assignment)
 *
 * Endpoints used BY Oblimap's own client (session auth):
 ...
 *
 * Endpoint called by Oblimap client after arriving from another app (no auth — IS the auth step):
...
// (Obliguard, Obliview, or Obliance) so all three can call validate-token.
// ─────────────────────────────────────────────────────────────────────────────
...
// Oblimap client calls this when user clicks "⇄ Obliguard".
...
// GET /api/sso/users
// Called BY the other app (Obliguard) with Bearer auth.
// Returns list of Oblimap users for admin pre-assignment of permissions.
// ─────────────────────────────────────────────────────────────────────────────
router.get('/users', async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const users = await db('users')
      ...
      .whereNull('foreign_source') // only expose local Oblimap users
...
// Called BY Oblimap's OWN client after the user arrives from another app.
// Supports Obliguard, Obliview, and Obliance as source apps.
...
router.post('/exchange', async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    if (obliviewCfg?.url  && obliviewCfg?.apiKey)  sources.push({ name: 'obliview',  url: obliviewCfg.url,  apiKey: obliviewCfg.apiKey  });
